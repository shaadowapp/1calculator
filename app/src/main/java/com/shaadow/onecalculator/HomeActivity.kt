    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        try {
            ...
            val searchAdapter = SearchResultsAdapter(searchResults) { result ->
                when (result) {
                    ...
                    is SearchResult.CalculatorItem -> {
                        ...
                        dialog.show(supportFragmentManager, "calculator_dialog")
                    }
                }
            }
            ...
            lifecycleScope.launch {
                try {
                    val db = HistoryDatabase.getInstance(this@HomeActivity)
            ...
            e.printStackTrace()
            Toast.makeText(this, "Error initializing home screen", Toast.LENGTH_SHORT).show()
            finish()
        }
    }
        ...
        lifecycleScope.launch {
            ...
            runOnUiThread {
                for (item in historyToShow) {
                    val box = LinearLayout(this@HomeActivity)
                    ...
                    val exprView = TextView(this@HomeActivity)
                    ...
                    val solView = TextView(this@HomeActivity)
                    ...
                    box.setOnClickListener {
                        Toast.makeText(this@HomeActivity, "Clicked: ${item.expression} = ${item.result}", Toast.LENGTH_SHORT).show()
                    }
    ...
    private fun addFlexButtons(containerId: Int, calculators: List<String>) {
        ...
        for ((i, label) in calculators.withIndex()) {
            ...
            button.setOnClickListener {
                    // Use popup dialog for all calculators
                    val dialog = CalculatorHostDialog.newInstance(label)
                    dialog.show(supportFragmentManager, "calculator_dialog")
                }
